...
class _AddPostTypeScreenState extends ConsumerState<AddPostTypeScreen> {
  ...
  void sharePost(){
    ...
    {
      ref.read(postControllerProvider.notifier).shareImagePost(context: context, title: titleController.text.trim(), selectedCommunity: selectedCommunity??communities[0], file: bannerFile,);
    ...
    return Scaffold(
      ...
      body: isLoading?const Loader():Padding(
        ...
        child: Column(
          ...
          if(isTypeImage)
            GestureDetector(onTap: selectBannerImage,
                child: DottedBorder(borderType: BorderType.RRect,radius: const Radius.circular(10),dashPattern: const[10,4],strokeCap: StrokeCap.round
                ,color: currentTheme.textTheme.bodyText2!.color!,child: Container(
          ...
          maxLines: 5,),
          if(isTypelink)
             TextField(controller: linkController,decoration:const  InputDecoration(
          ...
          const   Align(alignment: Alignment.topLeft,child: Text('Select Community')),
...
ref.watch(userCommunitiesProvider).when(data:(data){
  ...
  return DropdownButton(value: selectedCommunity??data[0],items: data.map((e) => DropdownMenuItem(value:e,child: Text(e.name))).toList(), onChanged: (val){
    setState(() {
