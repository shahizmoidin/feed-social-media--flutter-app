...
class _AddModsScreenState extends ConsumerState<AddModsScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ref.watch(getCommunityByNameProvider(widget.name)).when(data: (community)=>ListView.builder(
        ...
        itemBuilder: (BuildContext context, int index) {
         ...
         return ref.watch(getUserDataProvider(member)).when(data: (user){
          ...
          ctr++;
          return CheckboxListTile(value: uids.contains(user.uid), onChanged: (val){ 
            if(val!){
              addUids(user.uid);
            }
            else
          ...
          },title: Text(user.name),);},error: (error, stackTrace) =>ErrorText(error: error.toString()) , loading: ()=>const Loader());
